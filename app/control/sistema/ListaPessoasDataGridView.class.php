 ...
 */
class ListaPessoasDataGridView extends TPage
{
    ...
    public function __construct()
    {
        ...
        $category = $_GET["category"] ?? 8;
        $nameConfig = array(
            8 =>  ["name" => "Colaboradores"],
            6 =>  ["name" => "Lideranças"],
            2 =>  ["name" => "Assessores"],
        );

        $config = $nameConfig[$category];
        ...
        // TSession::setValue($this->activeRecord . '_filter_' . 'segmento', $category);

        // $criteria_default->add(new TFilter('segmento', '=', $category)); //liderancas
        ...
        // $this->addFilterField('segmento', '=', 'segmento'); // filterField, operator, formField
        ...
        $segmento   = new THidden('segmento');

        $segmento->setValue($category);
        ...
        $this->form->addFields([$segmento]); ///campo segmento
        ...
        $label = new TLabel($config["name"], '#7D78B6', 12, 'bi');
    ...
    public static function Delete($param)
    {
        try {
            ...
            $object = new SisMultiplicadores($key, FALSE); // instantiates the Active Record
